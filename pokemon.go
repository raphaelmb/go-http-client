...
func (c *Client) GetPokemonByName(ctx context.Context, pokemonName string) (Pokemon, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return Pokemon{}, PokemonFetchErr{Message: "non-200 status code from the API", StatusCode: resp.StatusCode}
	}

	var pokemon Pokemon
	err = json.NewDecoder(resp.Body).Decode(&pokemon)
	if err != nil {
		return Pokemon{}, PokemonFetchErr{Message: err.Error(), StatusCode: resp.StatusCode}
	}
